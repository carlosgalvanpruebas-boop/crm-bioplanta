...
- **Supabase/PostgREST tiene un tope de filas por consulta a nivel de proyecto (tipicamente 1000), que un `.limit()` mas alto pedido desde el cliente NO puede superar** (no da error, simplemente devuelve menos filas de las pedidas). Bug real detectado en 1.3 (resumen-venta.html): un `.select(...).order('fecha',{ascending:false}).limit(20000)` solo devolvia las ~1000 filas mas recientes, perdiendo meses completos del historico (agosto mostraba 54M en vez de ~934M reales). Cualquier consulta que pueda superar ~1000 filas (ventas, inventario si crece, etc.) debe traerse en paginas con `.range(desde, desde+PAGINA-1)` en un bucle hasta que una pagina vuelva incompleta -- no confiar en un solo `.limit()` alto.
- `sap_prefijos_excluidos` (exclusion de codigos SAP de otras areas del negocio, mismo centro de costo): la columna `prefijo` acepta tanto un prefijo de letras (ej. TUZAP, ACECRUDO -- excluye todo el grupo) como un codigo SAP completo exacto (ej. SER_04 -- excluye solo ese sub-codigo, sin arrastrar a SER_01/03/05/10/12). La funcion `estaExcluido()` (en api/cargar-ventas.js y carga-inventario.html) revisa primero coincidencia exacta y despues prefijo de letras -- no volver a usar solo `prefijoDe()` para esto.
- Grupos completos ya excluidos en `sap_prefijos_excluidos` (prefijo de letras): NOLI, SEM, AGROLAB (Fase 19), TUZAP, ACECRUDO, SER (Fase 30/31 -- Carlos confirmo excluir TODA la familia SER_xx, no solo SER_04/SER_06, porque son servicios de otra area aunque compartan centro de costo).
